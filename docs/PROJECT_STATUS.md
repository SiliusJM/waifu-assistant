...
1. Repetir/expandir la evidencia browser ahora que el runtime experimental está disponible, sin convertir el provisioning en una decisión productiva.
2. Ejecutar pruebas HTTPS controladas para HTTPS→HTTPS, HTTPS→HTTP y HTTPS→interno, y registrar el comportamiento del boundary.
...
La definición arquitectónica, el spike de providers/sandbox, los controlled tests, la ejecución externa, el egress boundary spike, el egress hardening, el decision-gate, la evidencia ejecutable, la evidencia HTTPS/DNS, la limitación de fixture TLS y el provisioning experimental del runtime browser fueron revisados y mergeados a `main` mediante PR #9, PR #10, PR #11, PR #12, PR #13, PR #14, PR #15, PR #16, PR #17, PR #18 y PR #19, respectivamente. El alcance actual sigue siendo preproducción.
