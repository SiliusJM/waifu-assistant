...
export class InteractionStream<T> implements AsyncIterable<T> {
  readonly capacity: number;
  ...
  constructor(options: InteractionStreamOptions = {}) {
    const capacity = options.capacity ?? 64;
    if (!Number.isInteger(capacity) || capacity < 1) {
      throw new RangeError('Interaction stream capacity must be a positive integer.');
    }
