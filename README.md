...
La siguiente etapa sigue siendo experimental, no implementación productiva. Con PR #29 ya está integrado el harness de navegador para DNS rebinding; el siguiente paso es ejecutar el experimento real dentro del laboratorio VirtualBox y conservar sus artefactos de DNS, referencia de reloj, navegación Chromium y delta de `nftables`. El resultado debe clasificarse estrictamente como `PASS`, `FAIL`, `LIMITATION` o `NOT EXECUTED`; un `PASS` no demuestra DNS pinning productivo ni cierra ADR-012.
