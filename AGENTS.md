...
El spike de network/host isolation (`docs/phase-08-network-host-isolation-capability-spike.md`) solo registra capacidades observadas y limitaciones: señales de Hyper-V/Firewall/vEthernet, Windows Sandbox no confirmado, WSL2 sin distribución y ausencia de evidencia OS-level. No habilitar features, crear VMs/switches, modificar Firewall ni cambiar políticas del host durante este checkpoint. ADR-012 continúa en `decision-gate`. La etapa posterior `docs/phase-08-network-host-isolation-evidence-results.md` fue revisada y mergeada mediante PR #25 y tampoco obtuvo evidencia OS-level ejecutable; el experimento queda `NOT EXECUTED`/`LIMITATION` hasta disponer de un entorno administrativo/dedicado reproducible. La evaluación posterior `docs/phase-08-remaining-evidence-assessment.md` fue revisada y mergeada mediante PR #26 y confirma que no debe repetirse evidencia bloqueada en el mismo entorno; ADR-012 continúa `decision-gate` hasta disponer de un entorno adecuado. La preparación `docs/phase-08-experimental-environment-readiness.md` fue revisada y mergeada mediante PR #27; cualquier ejecución futura debe cumplir su gate de preparación y mantenerse fuera del host de trabajo. La evidencia `docs/phase-08-dns-rebinding-real-evidence.md` fue revisada y mergeada mediante PR #28: el DNS rebinding controlado real dentro de VirtualBox es `PASS`, mientras el pinning productivo, el rebinding público y la validación de IP efectiva justo antes del socket dentro de BrowserProvider siguen pendientes. El PR #29 integró `docs/phase-08-browser-dns-rebinding-evidence.md`, un harness experimental de Chromium que correlaciona DNS1/DNS2 con `request.timing()`, orden temporal estricto, referencia de reloj `maxOffsetMs` y deltas diferenciales de `nftables`; su ejecución real en VirtualBox sigue pendiente y no autoriza `BrowserProvider` productivo.
